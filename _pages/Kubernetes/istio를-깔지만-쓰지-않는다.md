...
거의 안 쓰는 컴포넌트인데도 사고를 한 번 냈다. 원인은 Istio 자체가 아니라 **설치 순서**였다.
...
문제는 `istioctl operator init` 이 **명령이 끝나도 설치가 끝난 게 아니라는** 점이다.
...
| **`istiod` Available** (신규) | 컨트롤 플레인이 **실제로 동작**하는가 |
...
- ⚠️ 겪은 사고는 Istio 자체가 아니라 **설치 순서**였다. `istioctl operator init` 은
  즉시 반환되는데 파드는 아직 뜨는 중이라, 다음 컴포넌트가 먼저 시작됐다.
  **`istiod` Available 대기를 추가**해 해결
