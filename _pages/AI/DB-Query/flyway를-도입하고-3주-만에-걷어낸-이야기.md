...
`8c27ab14`에서 기존 스키마 덤프를 추출해 `V1__initial_schema.sql` 생성. 덤프를 그대로 넣는 것으로는 불충분:
...
`0e905c7e`에서 마이그레이션을 platform / feature_a / feature_b 모듈로 분리:
...
`9814fccc`에서 세 모듈의 pending 마이그레이션을 **글로벌 버전순으로 정렬해 하나씩 적용하는 인터리브 로직**을 FlywayConfig에 직접 구현.
...
- 대체 방식 — **dev DB dump import**. 로컬 환경은 dev의 덤프를 받아 구성
...
- 신호 발생 시점: FlywayConfig에 커스텀 로직이 붙기 시작한 때 (모듈별 분리 `0e905c7e`)
