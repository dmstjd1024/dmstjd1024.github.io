---
title: "백엔드 개발자 로드맵 2026"

categories:
  - Develop
tags:
  - Develop
  - Roadmap
  - Backend

date: 2026-08-22
thumbnail: "/assets/img/thumbnail/ect_thumbnail.jpg"
---

원본 로드맵의 세부 항목 132개를 하나도 합치지 않고 전부 그렸다.
큰 흐름만 보려면 아래 **전체 흐름** 도식 하나로 충분하고, 그 뒤 1~7단계가 세부 항목이다.

항목에 붙은 **대안**은 위의 기본 항목 대신 하나만 고르라는 뜻이고,
**순서 무관**은 아무 때나 익혀도 되는 항목이다. 표시가 없으면 기본으로 선택하는 항목이다.
`PostgreSQL` 과 `MySQL(대안)` 을 둘 다 학습할 필요는 없다.

---

전체 흐름
=====
-----

세부 항목을 제외한 큰 단계만 표시한다.

<div class="diagram">
{% include diagrams/backend-roadmap--stages.svg %}
</div>

---

1단계 · 기초
=====
-----

<div class="diagram">
{% include diagrams/backend-roadmap--s1.svg %}
</div>

백엔드 직군이라도 **HTML·CSS·JavaScript 는 기초 수준까지 포함된다.**
API 의 소비자가 프론트엔드이기 때문이다.

---

2단계 · 언어와 버전 관리
=====
-----

<div class="diagram">
{% include diagrams/backend-roadmap--s2.svg %}
</div>

**언어는 하나만 선택한다.** 나열된 항목은 선택지이며 전부 학습하는 대상이 아니다.
원본은 이 자리에 **"하나를 배우고 다음으로 넘어가기 전에 프로젝트를 많이 만들라"**
는 문구를 달아 뒀다.

---

3단계 · 데이터베이스와 API
=====
-----

<div class="diagram">
{% include diagrams/backend-roadmap--s3.svg %}
</div>

관계형 DB 는 **PostgreSQL 만 추천**이고 나머지 6개는 전부 대안이다.
`N+1 문제` 와 `마이그레이션` 은 특정 DB 에 종속되지 않는 공통 개념이라 같은 묶음에 들어 있다.
인증은 `JWT`·`OAuth` 가 기본이고 `OpenID`·`SAML` 은 순서 무관으로 분류돼 있다.

---

4단계 · 캐싱, 웹 서버, 보안
=====
-----

<div class="diagram">
{% include diagrams/backend-roadmap--s4.svg %}
</div>

원본은 이 구간이 끝나는 지점에 **"여기까지면 취업할 만큼은 안다.
이제 프로젝트를 만들며 실습하라"** 는 문구를 달아 뒀다.
로드맵 전체 분량의 절반도 안 되는 지점이다.

---

5단계 · AI (2026년 신규)
=====
-----

이전 로드맵에는 없던 영역이다. 배치 위치는 맨 뒤가 아니라 **기본기 직후 · 심화 직전**이다.

<div class="diagram">
{% include diagrams/backend-roadmap--s5.svg %}
</div>

**AI 도구의 사용과 AI 기능의 구현은 별도 항목으로 구분된다.**
백엔드 직군의 요구 범위는 후자, 즉 스트리밍·Function Calling 을 이용한 기능 구현이다.

---

6단계 · 심화
=====
-----

<div class="diagram">
{% include diagrams/backend-roadmap--s6.svg %}
</div>

---

7단계 · 규모 대응
=====
-----

<div class="diagram">
{% include diagrams/backend-roadmap--s7.svg %}
</div>

`관측성(Observability)`, `서킷 브레이커`, `백프레셔`, `스로틀링` 등
장애 대응·복원력 관련 항목이 이 구간에 배치돼 있다.

NoSQL 은 원본에서 **성격별로 묶여** 있다. 도식에는 이름만 폈으므로 분류를 적어 둔다.

| 분류 | 항목 |
|---|---|
| 문서 | MongoDB · CouchDB |
| Key-Value | Redis · DynamoDB |
| 컬럼 | ClickHouse · Cassandra · ScyllaDB |
| 그래프 | Neo4j · AWS Neptune · DGraph |
| 시계열 | Influx DB · TimescaleDB |
| 실시간 | Firebase · RethinkDB |

원본은 이 구간에 **"대부분은 평생 쓸 일이 없을 수도 있다.
무엇이 있고 언제 쓰는지만 알아 두라"** 는 문구를 달아 뒀다.

---

Docker 와 Kubernetes 는?
=====
-----

로드맵을 보면 컨테이너 자리에 Docker·Kubernetes 가 있는데,
이 둘은 **로드맵 안의 학습 항목이 아니라 별도 로드맵으로 빠지는 버튼**이다.

<div class="diagram">
{% include diagrams/backend-roadmap--containers.svg %}
</div>

백엔드 로드맵 내부의 학습 항목이 아니라, 각각 독립된 로드맵으로 연결되는 분기점이다.

---

정리
=====
-----

- 기본기 구간은 **인터넷 기초 → 언어 → Git → DB → API → 캐싱 → 웹 서버** 까지다.
- **AI 는 기본기 직후**에 배치돼 있다. 이전 로드맵에는 없던 영역이다.
- **대안 항목은 택1**이며 전체 학습 대상이 아니다. 세부 항목 132개 중 28개가 대안이다.
- Docker·쿠버네티스는 별도 로드맵으로 분리돼 있다.

---

참고
=====
-----

- [roadmap.sh/backend](https://roadmap.sh/backend) — 원본 로드맵 (2026-02-07 갱신본 기준, 토픽 23개 · 세부 항목 132개)

원본에는 다른 로드맵으로 빠지는 버튼이 이 글에 그린 것 말고도 더 있다.
학습 항목이 아니라 분기점이라 도식에는 넣지 않았다.

| 위치 | 나가는 로드맵 |
|---|---|
| 컨테이너 | [Docker](https://roadmap.sh/docker) · [쿠버네티스](https://roadmap.sh/kubernetes) |
| 보안 | [API 보안 모범 사례](https://roadmap.sh/best-practices/api-security) |
| AI | [프롬프트 엔지니어링](https://roadmap.sh/prompt-engineering) · [AI 에이전트](https://roadmap.sh/ai-agents) |
| 아키텍처 | [시스템 디자인](https://roadmap.sh/system-design) · [소프트웨어 설계](https://roadmap.sh/software-design-architecture) |
| 규모 대응 | [DevOps](https://roadmap.sh/devops) · [풀스택](https://roadmap.sh/full-stack) |
| 실습 | [프로젝트 아이디어](https://roadmap.sh/backend/projects) (입문 · 중급) |
